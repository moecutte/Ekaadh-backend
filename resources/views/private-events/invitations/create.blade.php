    ...
    <div class="relative max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-12">
        ...
        <div class="mt-5 mb-8 flex flex-wrap items-start gap-5">
            ...
            <div class="min-w-0 flex-1">
                <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-brand mb-2">Guest delivery</p>
                <h1 class="text-3xl font-extrabold tracking-tight text-ink">Send invitations</h1>
                <p class="text-sm text-mute mt-2">{{ $event->title }} · {{ $remaining }} seats left · each guest gets one invitation link.</p>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-5 gap-6" x-data="inviteForm()">
            <form method="POST" action="{{ route('private-events.invitations.store', $event) }}"
                ...
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <h3 class="text-sm font-extrabold text-ink">Add guests</h3>
                        <p class="text-[11px] text-mute mt-0.5">Name, phone, quantity, and ticket type per row.</p>
                    </div>
                    <span class="text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full bg-brand-soft text-brand" x-text="rows.length + ' guest' + (rows.length === 1 ? '' : 's')"></span>
                </div>
                    ...
                    <template x-for="(row, index) in rows" :key="index">
                        <div class="relative rounded-2xl border border-slate-100 bg-gradient-to-br from-slate-50/80 to-white p-4 space-y-3">
                            <div class="flex items-center justify-between">
                                <span class="text-[10px] font-extrabold uppercase tracking-wider text-mute" x-text="'Guest ' + (index + 1)"></span>
                                <button type="button" class="text-[11px] font-bold text-red-400 hover:text-red-600 transition-colors" @click="remove(index)" x-show="rows.length > 1">Remove</button>
                            </div>
                            <div class="grid grid-cols-12 gap-3">
                                <div class="col-span-12 sm:col-span-5">
                                    <label class="text-[11px] font-bold text-mute block mb-1">Name</label>
                                    <input class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/15 transition" :name="'guests['+index+'][name]'" x-model="row.name" placeholder="Guest name">
                                </div>
                                <div class="col-span-12 sm:col-span-4">
                                    <label class="text-[11px] font-bold text-mute block mb-1">Phone *</label>
                                ...
                                <div class="col-span-4 sm:col-span-3">
                                    <label class="text-[11px] font-bold text-mute block mb-1">Qty</label>
                                ...
                                <div class="col-span-8 sm:col-span-12">
                                    <label class="text-[11px] font-bold text-mute block mb-1">Ticket type</label>
            ...
            <form method="POST" action="{{ route('private-events.invitations.store', $event) }}" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <h3 class="text-sm font-extrabold text-ink">Upload CSV</h3>
                    <p class="text-[11px] text-mute mt-1 leading-relaxed">Bulk-import guests. Columns: <code class="bg-slate-100 px-1.5 py-0.5 rounded text-[10px] font-semibold">phone, name, quantity, ticket_type</code></p>
                </div>
                <div>
                    <label class="text-xs font-bold text-mute block mb-1.5">Default ticket type</label>
                ...
                <label class="flex flex-col items-center justify-center gap-2 w-full rounded-xl border border-dashed border-slate-200 bg-slate-50/40 px-4 py-8 cursor-pointer hover:border-brand/40 hover:bg-brand-soft/30 transition">
                    <span class="text-xs font-semibold text-mute">Drop CSV or click to browse</span>
