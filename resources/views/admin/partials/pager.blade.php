@php
    ...
    $perPageOptions = $perPageOptions ?? [10, 15, 20, 50];
...
@if($paginator->total() > 0)
    <div class="px-4 py-3 border-t border-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <p class="text-xs text-mute">
            Showing <span class="font-bold text-ink">{{ $paginator->firstItem() }}–{{ $paginator->lastItem() }}</span>
            of <span class="font-bold text-ink">{{ number_format($paginator->total()) }}</span>
        </p>
            ...
            @if($showPerPage)
                <form method="GET" class="flex items-center gap-1.5">
                    @foreach(request()->except(['page', 'per_page']) as $key => $value)
                        ...
                        @endif
                    @endforeach
                    <label class="text-[11px] font-bold uppercase tracking-wide text-mute">Rows</label>
            ...
            @if($paginator->hasPages())
                <nav class="flex items-center gap-1" aria-label="Pagination">
                    @if($paginator->onFirstPage())
                        <span class="px-2 py-1 rounded-lg text-xs font-bold text-slate-300">Prev</span>
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" class="px-2 py-1 rounded-lg text-xs font-bold text-brand hover:bg-brand/5">Prev</a>
                    @endif
                    ...
                    @if($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" class="px-2 py-1 rounded-lg text-xs font-bold text-brand hover:bg-brand/5">Next</a>
                    @else
                        <span class="px-2 py-1 rounded-lg text-xs font-bold text-slate-300">Next</span>
                    @endif
                </nav>
            @endif
        </div>
