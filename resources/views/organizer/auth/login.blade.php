...
<form method="POST" action="{{ route('organizer.login') }}" class="space-y-4">
    ...
    <div>
        ...
        <input name="login" value="{{ old('login', 'organizer@ekaadh.com') }}" required class="w-full rounded-xl bg-slate-50 border border-slate-200 px-4 py-3 text-sm outline-none focus:border-brand">
    </div>
    <div>
