...
@section('actions')
    @if(auth()->user()->unreadNotifications()->exists())
        <form method="POST" action="{{ route('organizer.notifications.read-all') }}">
            @csrf
...
<div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
    <div class="divide-y divide-slate-50">
        @forelse($notifications as $note)
            ...
            <a href="{{ route('organizer.notifications.open', $note->id) }}" class="block px-5 py-4 hover:bg-slate-50 {{ $note->read_at ? '' : 'bg-brand/5' }}">
        ...
        @empty
            <p class="px-5 py-12 text-center text-sm text-mute">No notifications yet. You’ll see event approvals, ticket sales, and payouts here.</p>
        @endforelse
    </div>
</div>
