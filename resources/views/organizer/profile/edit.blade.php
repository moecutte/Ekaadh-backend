...
<form method="POST" action="{{ route('organizer.profile.update') }}" enctype="multipart/form-data" class="max-w-2xl space-y-5" x-data="profilePhoto()">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <h3 class="text-sm font-bold mb-1">Profile image</h3>
        <p class="text-xs text-mute mb-4">Shown on your dashboard and on public event pages.</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="phone" value="{{ old('phone', $user->phone) }}" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2.5 text-sm outline-none focus:border-brand">
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
        ...
        <div>
            ...
            <input name="business_name" value="{{ old('business_name', $profile->business_name) }}" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2.5 text-sm outline-none focus:border-brand">
        </div>
        <div class="grid sm:grid-cols-2 gap-4">
            <div>
                <label class="text-xs font-bold text-mute block mb-1.5">Business phone</label>
                <input name="business_phone" value="{{ old('business_phone', $profile->business_phone) }}" class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2.5 text-sm outline-none focus:border-brand">
            </div>
            <div>
                <label class="text-xs font-bold text-mute block mb-1.5">City</label>
                <input name="city" value="{{ old('city', $profile->city) }}" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2.5 text-sm outline-none focus:border-brand">
            </div>
        </div>
        <div>
            <label class="text-xs font-bold text-mute block mb-1.5">About the business</label>
            <textarea name="business_description" rows="4" required maxlength="500" class="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2.5 text-sm outline-none focus:border-brand resize-y">{{ old('business_description', $profile->business_description) }}</textarea>
        </div>
    </div>
    @endif
