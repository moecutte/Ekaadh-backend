    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 w-full">
        <div class="max-w-2xl">
            ...
            <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white leading-[1.1] mb-5">
                Find Events<br>
                <span class="text-brand">Near You</span>
            </h1>
            <p class="text-slate-300 text-base sm:text-lg mb-8 leading-relaxed max-w-lg">
                Discover concerts, conferences, cultural shows and more. Pay instantly with Zaad or eDahab — no bank card needed.
            </p>
            ...
            <form action="{{ route('events.index') }}" method="GET" class="bg-white rounded-2xl shadow-2xl max-w-2xl flex items-stretch overflow-hidden">
                <div class="flex items-center gap-2 flex-1 px-4 py-1 min-w-0">
                    ...
                    <input type="search" name="q" placeholder="Search events..." class="flex-1 outline-none text-sm text-ink placeholder-slate-400 bg-transparent min-w-0 py-3">
                </div>
                ...
                <button type="submit" class="bg-brand hover:bg-brand-dark text-white font-bold px-6 text-sm shrink-0 transition-colors rounded-r-2xl">Search</button>
            </form>
        </div>
    </div>
</section>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <section class="pt-10 mb-14">
        <div class="flex items-center gap-3 mb-5">
            <h2 class="text-2xl font-extrabold text-ink">Featured &amp; Trending</h2>
            <span class="bg-brand/10 text-brand text-xs font-extrabold px-2.5 py-0.5 rounded-full">HOT</span>
        </div>
                ...
                <a href="{{ route('events.show', $event->slug) }}"
                   class="relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all group border border-slate-100 {{ $i === 0 ? 'md:col-span-2' : '' }}">
                    <div class="relative overflow-hidden bg-slate-200 {{ $i === 0 ? 'h-72' : 'h-52' }}">
                        ...
                        <div class="absolute top-3 left-3 bg-brand text-white text-xs font-extrabold px-3 py-1 rounded-full">FEATURED</div>
                        <div class="absolute bottom-0 left-0 right-0 p-5">
                            ...
                            <div class="flex flex-wrap items-center gap-3 mt-1.5 text-white/75 text-xs">
                                ...
                                @if($price !== null)<span class="font-extrabold text-brand">From ${{ number_format((float)$price, 0) }}</span>@endif
                            </div>
                        </div>
                    </div>
                </a>
    ...
    <div class="flex gap-2 overflow-x-auto pb-5 hide-scrollbar">
        <a href="{{ route('home') }}" class="shrink-0 px-4 py-2 rounded-full text-sm font-bold bg-brand text-white shadow-sm">All</a>
        @foreach($categories as $cat)
    ...
    <section class="mb-14">
        <div class="flex items-center justify-between mb-5">
            <h2 class="text-2xl font-extrabold text-ink">Upcoming Events</h2>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            @empty
                <p class="text-mute col-span-full py-8">No upcoming events yet.</p>
            @endforelse
        </div>
    </section>
    ...
    <section class="mb-16">
        <div class="text-center mb-10">
            <h2 class="text-2xl font-extrabold text-ink mb-2">How It Works</h2>
            <p class="text-mute text-sm">Get your tickets in 3 simple steps</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            @foreach([
                ['1', 'Browse Events', 'Discover events across Somalia & Somaliland — concerts, conferences, culture, and more.', 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z'],
                ['2', 'Buy Instantly', 'Pay securely with Zaad or eDahab. No bank card needed. Confirmed in seconds.', 'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z'],
                ['3', 'Get Your QR Ticket', 'Receive your e-ticket via WhatsApp, SMS, and email. Show the QR at the door.', 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
    ...
    <section class="mb-16">
        <div class="bg-ink rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
                <h3 class="text-white font-extrabold text-xl mb-1">Pay with what you know</h3>
                <p class="text-slate-400 text-sm">No bank account or credit card required. Just your Zaad or eDahab number.</p>
            </div>
