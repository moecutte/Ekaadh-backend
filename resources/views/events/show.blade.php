    ...
    <div class="absolute bottom-0 left-0 right-0 p-5 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap gap-4 text-white/80 text-sm">
            <span class="flex items-center gap-1.5">
                ...
                {{ $event->event_date?->format('M j, Y') }}@if($event->event_time) at {{ date('g:i A', strtotime($event->event_time)) }}@endif
            </span>
            <span class="flex items-center gap-1.5">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <a href="{{ route('events.index') }}" class="inline-flex items-center gap-1 text-sm font-bold text-mute hover:text-brand mb-6">&larr; Back to events</a>

    <div class="grid lg:grid-cols-5 gap-8">
        <div class="lg:col-span-3 space-y-5">
            <div class="bg-white rounded-2xl border border-slate-100 p-6">
                <h2 class="text-xl font-extrabold text-ink mb-4">About This Event</h2>
                <div class="text-mute leading-relaxed text-sm whitespace-pre-line">{{ $event->description }}</div>
            </div>

            @if($event->organizer)
            <div class="bg-white rounded-2xl border border-slate-100 p-6">
                <h2 class="text-xl font-extrabold text-ink mb-4">Organizer</h2>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-mute">Verified Organizer</p>
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="bg-white rounded-2xl border border-slate-100 p-6">
                <h2 class="text-xl font-extrabold text-ink mb-4">Venue</h2>
        ...
        <div class="lg:col-span-2" x-data="eventTickets()">
            <div class="bg-white rounded-2xl border border-slate-100 shadow-lg p-5 sticky top-24">
                <div class="flex items-center justify-between mb-1">
                    <p class="text-xs text-mute">Starting from</p>
                    <span class="text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Tickets Available</span>
                </div>
                <p class="text-3xl font-extrabold text-ink mb-5">
                    ${{ number_format((float) ($starting ?? 0), 0) }}
                    <span class="text-sm font-normal text-mute">/ ticket</span>
                </p>
                ...
                <div class="space-y-3 mb-4">
                    @foreach($event->ticketTypes as $type)
                        ...
                        <div class="p-3 border border-slate-100 rounded-xl">
                            ...
                            <p class="text-xs text-mute">{{ $type->remaining() }} remaining</p>
                        </div>
                    @endforeach
                </div>
                ...
                <div x-show="ticketCount > 0" x-cloak class="flex items-center justify-between py-3 border-t border-slate-100 mb-4">
                    <span class="text-sm font-bold text-ink">
                        Subtotal (<span x-text="ticketCount"></span> ticket<span x-show="ticketCount !== 1">s</span>)
                    </span>
                ...
                <a
                    ...
                    x-text="ticketCount === 0 ? 'Select Tickets to Continue' : ('Proceed to Checkout · $' + subtotal.toFixed(0))"
                ></a>
    ...
    @if($related->isNotEmpty())
        <section class="mt-16">
            <h2 class="text-xl font-extrabold mb-5">You might also like</h2>
...
<div class="fixed bottom-0 left-0 right-0 lg:hidden bg-white border-t border-slate-100 px-4 py-3 flex items-center gap-3 shadow-xl z-40" x-data="eventTickets()">
    <div class="shrink-0">
        <p class="text-xs text-mute">From</p>
